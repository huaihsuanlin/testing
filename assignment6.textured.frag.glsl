...
vec3 shadeAmbientLight(Material material, AmbientLight light) {
    ...
    vec3 scale_kA = texture(material.map_kD, o_texture_coord).rgb;
    return light.color * light.intensity * material.kA * scale_kA;
}
...
vec3 shadeDirectionalLight( Material material, DirectionalLight light, vec3 normal, vec3 eye, vec3 vertex_position) {
    ...
    // HINT: Refer to http://paulbourke.net/dataformats/mtl/ for details
    // HINT: Parts of ./shaders/phong.frag.glsl can be re-used here
    vec3 scale_kD = texture(material.map_kD, o_texture_coord).rgb;
    float scale_shininess = texture(material.map_nS, o_texture_coord).r;
    vec3 result = vec3(0);
    if (light.intensity == 0.0)
        return result;
    ...
    // Specular
    vec3 R = reflect(L, N);
    result += pow( max(dot(R, V), 0.0), material.shininess * scale_shininess) * light.color * light.intensity * material.kS;
